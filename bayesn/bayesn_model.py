...
class SEDmodel(object):
    ...
    def train_model_v2(self, obs, weights):
        ...
        Wc = numpyro.sample('Wc_red', dist.MultivariateNormal(W_mu3, jnp.eye(N_knots - 4)))
        Wc = jnp.insert(Wc, N_l_knots + 2, Wc[N_l_knots + 1] - 1)
        Wc = jnp.insert(Wc, 2 * N_l_knots + 1, Wc[N_l_knots + 1])
        Wc = jnp.insert(Wc, 3 * N_l_knots + 1, Wc[N_l_knots + 1])
        Wc = jnp.insert(Wc, 4 * N_l_knots + 1, Wc[N_l_knots + 1])
    ...
    def train_model_v2_fixW1(self, obs, weights):
        """
        Numpyro model used for training to learn global parameters, assuming a single global RV
        ...
        M0 = numpyro.sample('M0', dist.Uniform(-20, -19))
        sigma_theta = 1. # numpyro.sample('sigma_theta', dist.Uniform(0, 2))
        sigma_cint = numpyro.sample('sigma_cint', dist.Uniform(0, 0.3))
    ...
    def initial_guess(self, args, reference_model='M20_model'):
        ...
        if args['mode'] == 'training_v2':
            ...
            param_init['W0_red'] = jnp.array([-0.33002263, -0.16611046, -0.15744324, -0.09390546, -0.03231157,
        1.3280425 , -0.19466828, -0.02751061,  0.06483366,  0.10507142,
        0.7616973 ,  0.06440454,  0.0324329 ,  0.04849258,  0.07480597,
        0.07260877, -0.01146984, -0.01484789,  0.09001668,  0.00686859,
        0.03435605,  0.17239851,  0.4074164 , -0.5428639 ,  0.14652443,
        0.05156759,  0.16785675,  0.2171385 ,  0.4386788 , -0.18444404,
        0.01115638, -0.05531065,  0.03697701,  0.09223546,  0.01527985])
            param_init['W1_red'] = jnp.array([0.8711952 ,  2.636076  ,  1.5632235 ,  1.8474456 ,  1.2132231 ,
       -0.15008132,  1.8897369 ,  0.652902  ,  1.002607  ,  0.18983603,
        1.040541  ,  1.0423782 ,  2.2666173 ,  0.9363808 ,  0.8677329 ,
        0.52974534, -0.8885846 ,  0.7058834 ,  2.5666683 ,  0.7387118 ,
       -0.26552123,  0.50841576,  3.8533642 ,  2.429559  ,  2.4928205 ,
        2.570306  ,  2.0316968 ,  2.1310935 ,  5.4361405 ,  0.40265954,
        1.5472957 ,  1.6011423 ,  1.7436783 ,  2.8843803 ,  2.201987])
            param_init['Wc_red'] = jnp.array([0.7509145 ,  0.53643435, -0.66996735, -0.09658708, -0.32999915,
       -0.20513143, -5.377417  ,  0.28724128, -0.03760658, -0.04521886,
        1.0539091 ,  2.8864968 ,  0.34401667, -0.00900129,  0.24898171,
       -0.7802849 , -0.41675654, -0.26833454,  0.4108602 , -0.5147132 ,
    ...
    def run(self, args, cmd_args):
        ...
        if args['mode'].lower() == 'training_globalrv':
            ...
            nuts_kernel = NUTS(self.train_model_v2, adapt_step_size=True, target_accept_prob=0.8,
                               ...
                               dense_mass=False, find_heuristic_step_size=False, regularize_mass_matrix=False,
                               step_size=0.1)
        elif args['mode'].lower() == 'training_v2_fixW1':
            nuts_kernel = NUTS(self.train_model_v2_fixW1, adapt_step_size=True, target_accept_prob=0.8,
                               init_strategy=init_strategy,
                               dense_mass=False, find_heuristic_step_size=False, regularize_mass_matrix=False,
                               step_size=0.1)

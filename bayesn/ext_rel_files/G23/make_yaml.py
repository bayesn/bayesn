...
for subdomain, s, exp, complement in zip(
    (2, 3),
    ...
    coeffs["A"]["poly"][subdomain] = s * w_poly
    coeffs["A"]["rem"][subdomain] = s * w_rem
    coeffs["A"]["div"][subdomain] = w_div
    exps[subdomain] = exp
...
# This lets us add them in one wavelength subdomain rather than iterating for each Drude
...
for var, subdomain, s, exp in zip("AB", (8, 9), (scale, b_ir_scale), (alpha_1, -b_ir_exp)):
    coeffs[var]["poly"][subdomain] = s * w_poly
    coeffs[var]["rem"][subdomain] = s * w_rem
    coeffs[var]["div"][subdomain] = w_div
    exps[subdomain] = exp
...
# However, this magnifies floating point errors more than two subdomain would
for i in (15, 16):
...
# If a(x) is already populated, it's easier to make a new subdomain than to combine rational fns.
for subdomains in (
    ...
    b_subdomain, a_subdomain = subdomains
    coeffs["A"]["poly"][a_subdomain] = -coeffs["B"]["poly"][b_subdomain] / 3.1
    coeffs["A"]["rem"][a_subdomain] = -coeffs["B"]["rem"][b_subdomain] / 3.1
...
with open("BAYESN.YAML", "w") as f:
    ...
    f.write(f"A_EXP: [{', '.join(str(x) for x in exps)}]\n")
    f.write(f"B_EXP: [{', '.join(str(x) for x in exps)}]\n")
    for arr, name in zip(
        (
            wns,
            coeffs["A"]["poly"],
        ...
        (
            "SUBDOMAINS",
            *np.array([[f"{var}_{term}_COEFFS" for var in "AB"] for term in ("POLY", "REMAINDER", "DIVISOR", "DRUDE")]).flatten(),
        ),
    ):
        name = name.replace("DRUDE_COEFFS", "DRUDE_PARAMS")
        f.write(f"{name}:\n")
        for coeffs in arr:
